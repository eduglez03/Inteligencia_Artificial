...
/*
// Metodo que obtiene un valor aleatorio de un vector
int obtenerValorAleatorio(const std::vector<int>& vec) {
  // Inicializar la semilla para generar números aleatorios
  std::srand(std::time(0));

  // Obtener un índice aleatorio dentro del rango del vector
  int indiceAleatorio = std::rand() % vec.size();

  // Devolver el valor en el índice aleatorio
  return vec[indiceAleatorio];
}

// Obtiene un valor aleatorio en un rango del vector
int obtenerValorAleatorioEnRango(const std::vector<int>& vec) {
  // Inicializar la semilla para generar números aleatorios
  std::srand(std::time(0));

  // Obtener un índice aleatorio dentro del rango especificado
  int indiceAleatorio = 0 + std::rand() % (vec.size() - 0 + 1);
