...
class Astar {
  public:
    ...
    std::vector<Nodo*> obtenerHijos(const Nodo* padre);
...
Nodo* Astar::menorCoste(const std::vector<Nodo*>& nodos) {
  ...
  auto comparador = [](const Nodo* a, const Nodo* b) {
...
std::vector<Nodo*> Astar::obtenerHijos(const Nodo* padre) {
    ...
    if (hijo != padre->get_coordenadas()) {
      if (comprobarRango(hijo)) {
        if (i % 2 == 0) {
          ...
          Nodo* nuevo_nodo = new Nodo(hijo, g_susesores, 0, 0, padre->get_coordenadas());
